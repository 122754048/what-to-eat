...
| **AI** | 前端直连 Cloudflare AI | 前端根据心情/健康目标选择菜系；后端使用预置推荐文案 |
...
V1版本使用预置推荐文案，不调用AI。前端已直接调Cloudflare AI根据心情/健康目标选择菜系。

后续版本可扩展为调用Workers AI生成个性化推荐理由。
...
**待确认事项:**
1. ✅ 设备ID：前端生成UUID，服务端不做限制
2. ✅ 用户偏好：需要持久化，支持用户设置
3. ✅ 多语言：当前版本仅支持中文
4. ✅ AI架构：前端直连Cloudflare AI，后端使用预置文案
