...
详情页是用户点击滑动卡片或从列表选择后进入的核心页面，包含以下4个必须要素：

**布局（从上到下）**:
1. **Hero区域**: 全宽食物大图，高度占屏幕35%
2. **基本信息卡片**:
   - 菜品名称: `.title2` Semibold 24pt
   - 餐厅名 + 地点图标: `.subheadline` 16pt
   - 评分: 5个绿色星星 `.foregroundStyle(.green)`
3. **推荐理由**: `.body` 16pt，AI生成，带🤖图标
4. **热量信息**:
   - 标签 "卡路里"
   - 范围值: "350-450 kcal"
   - 绿色小标签样式 `.padding(4).background(Color.green.opacity(0.1)).cornerRadius(4)`
5. **关联搭配菜品** (横向滚动):
   - 标题 "配菜推荐" + 餐具图标
   - 2-3张小卡片，图片+菜名
6. **关联搭配饮品** 🆕 (v2.0+):
   ...
   - 每张: 饮品照片 + 名称 + 小标签
7. **底部操作栏**:
   - 左: "再来一次" `.buttonStyle(.bordered)`
   - 右: "去吃！" `.buttonStyle(.borderedProminent)` 橙色背景
